<!--
Scientific Writer configuration, TRIMMED for the Bulk_GWA_Manuscript repository.
Original template: ~/.claude/plugins/cache/claude-scientific-writer/claude-scientific-writer/2.21.0/templates/
Sections removed or scoped are listed under "Local scope" below.
-->

# Project instructions

## Local scope (takes precedence over everything below)

This repository is a **finished analysis pipeline for a specific manuscript**, not a
greenfield document generator. The Scientific Writer guidance below applies to
**new standalone documents** written into `writing_outputs/`. It does NOT govern
the manuscript itself.

For the manuscript, the repository's own conventions win:

- **Figures come from versioned R scripts**, not from image generation. 28 figure
  scripts (`scripts/Figure*.R`, `scripts/SUPP_FIG*.R`) each write their own figure
  and assert their pinned literals. 17 `scripts/DIAG_*.R` are diagnostics, exempt
  from the figure-script rules. Never generate a manuscript figure with
  `scientific-schematics` or `generate-image`.
- **Prose lives in tracked text files**: `METHODS.txt`, `FIGURE_CAPTIONS.txt`,
  `MANUSCRIPT_CAPTIONS.txt`, `DATA_AVAILABILITY.md`, `FIGURE_REPORT.Rmd`.
  Not LaTeX. There is no `writing_outputs/` tree for manuscript work.
- **Three checkers gate every change**, wired into `githooks/pre-push`:
  `scripts/check_repo_invariants.sh` (exit status = number of failed checks),
  `scripts/check_figure_lists.R`, `scripts/check_manuscript_numbers.py`.
  Run them after editing any figure script, caption, or methods text.
- **Figure scripts must build from a clone** and must not reference `data/`.
- **A check must not have side effects on tracked files.**
- **Numbers in prose must be produced by code**, not asserted. If a checker cannot
  match a number, surface it in `FIGURE_REPORT.Rmd` rather than adding an exception.

Ask before destructive or hard-to-reverse actions, and stop when a decision is the
user's to make. Report what was actually verified, including failures.
...
**Default Format (new standalone documents only):** LaTeX with BibTeX citations
unless otherwise requested. Does not apply to this repository's manuscript, which
uses tracked plain-text files.
...
**Completion:** Finish the whole task rather than handing back partial work, and
say explicitly what was left out and why if some part is blocked. This does not
override checking in on decisions that are the user's, or confirming before
...
2. **Present Brief Plan and Execute Immediately**
   ...
   - Begin execution without waiting for approval when the request is
     unambiguous; ask first when different readings would mean materially
     different work
...
3. **Peer Review (on request)** - Use the peer-review skill when asked, save as `PEER_REVIEW.md`
...
For **new standalone documents**, generate figures where they carry real
information, using `scientific-schematics` for diagrams and `generate-image` for
illustrative visuals. Let the content decide how many; there is no minimum, and a
generated graphical abstract is optional.

**For this repository's manuscript, do not generate figures at all.** Every figure
is produced by a versioned R script that asserts its own pinned numbers. Edit the
script, rerun it, and rerun the checkers.
...
- [ ] PEER_REVIEW.md completed (if peer review was requested)
...
- **LaTeX is the default format for new standalone documents**
...
- **Complete tasks fully**, while still confirming decisions that are the user's
