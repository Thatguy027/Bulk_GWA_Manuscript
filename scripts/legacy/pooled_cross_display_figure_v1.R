## pooled_cross_display_figure.R -------------------------------------------
## The single display figure: pooled GWAS above, and for each cross the two
## contrasts that separate a general RNAi-response locus from a
## knockdown-specific one, overlaid in one panel. All six chromosomes.
##
##   Rscript scripts/pooled_cross_display_figure.R
##
## Why overlay rather than stack
## -----------------------------
## "General" versus "knockdown-specific" is not a property of one scan; it is a
## comparison BETWEEN two contrasts of the same cross:
##
##   HT115 vs pos-1   picks up everything that changes the pos-1 response,
##                    including loci that change dsRNA uptake generally
##   mig-6 vs pos-1   cancels anything acting equally on both knockdowns, so
##                    only knockdown-specific loci survive
##
## so a locus is general where the first is high and the second is flat, and
## knockdown-specific where the reverse holds. Drawing the two in the same panel
## puts that comparison in one place instead of making the reader track between
## rows. FIG1 remains the all-contrast, all-chromosome view.
##
## Colour is shared between the GWAS row and the LOD rows: the pos-1 trait and
## the HT115-vs-pos-1 contrast are purple, the mig-6 trait and the
## mig-6-vs-pos-1 contrast blue.
##
## The only genomic annotation carried across panels is a 1 Mb window either
## side of each genome-wide significant pooled GWAS peak, drawn identically in
## every row, so the figure marks what the association scan found and nothing
## the reader has to look up elsewhere.
## ---------------------------------------------------------------------------
...
trait_lab <- tibble(
  chrom  = fct_chr(c("I", "I")),
  pos.mb = 0.3,
  y      = c(b$GWAS_BF + 1.1, -(b$GWAS_BF + 1.1)),
...
ggsave(file.path(OUT, "FIG16_display_general_vs_specific.pdf"), fig,
       width = 13, height = 8.2, device = cairo_pdf)
ggsave(file.path(OUT, "FIG16_display_general_vs_specific.png"), fig,
       width = 13, height = 8.2, dpi = 300, bg = "white")
cat("wrote FIG16_display_general_vs_specific.{pdf,png}\n")
