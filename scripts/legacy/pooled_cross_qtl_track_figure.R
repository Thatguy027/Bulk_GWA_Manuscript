## pooled_cross_qtl_track_figure.R -----------------------------------------
## The compressed display figure: the pooled GWAS Manhattan, with the cross QTL
## reduced to interval tracks laid over it -- mig-6-specific above, pos-1-
## specific below, coloured by cross.
##
##   Rscript scripts/pooled_cross_qtl_track_figure.R
##
## What the two tracks contain
## ---------------------------
## The two tracks come from DIFFERENT scans, on purpose.
##
##   upper   mig-6 vs pos-1        the mig-6-SPECIFIC QTL. A locus acting equally
##                                 on both knockdowns cancels in this contrast,
##                                 so what survives is knockdown-specific, and it
##                                 is localised without the shared component
##                                 dragging the peak around.
##   lower   HT115 vs pos-1        the pos-1 response, shared component included.
##
## Sourcing the upper track from HT115 vs mig-6 instead was tried and rejected:
## that scan mixes the chromosome-V specific locus with the broad shared signal,
## so its chromosome-V peak lands at 10.76 Mb, 3.9 Mb from the pooled mig-6 GWAS
## peak. The contrast puts it at 13.81 Mb, inside the 1 Mb window around that
## peak. Sourcing the LOWER track from the contrast was also tried and rejected,
## for the opposite reason: the chromosome III QTL of the pos-1 response (LOD 710
## in N2 x XZ1516, 140 in JU1793 x JU2466) cancels in the contrast, so deriving
## the lower track from it deleted the chromosome III overlap entirely.
##
## Because the upper track is a contrast, it reports only that the two knockdowns
## differ, not which one carries the effect. The printed table gives the two
## HT115 LODs at every upper-track peak so the mig-6 attribution can be checked.
##
## One interval per chromosome per scan (MAX_PEAKS = 1). Greedy multi-peak
## calling was implemented and then switched off, because chromosome V in the
## N2 x XZ1516 HT115-vs-mig-6 scan is not two peaks but one broad plateau running
## from about 10.5 to 15 Mb: the maximum is 806 at 10.76 Mb, and the trace stays
## between 600 and 750 for the next 3 Mb. Greedy calling therefore returns
## 10.76 and then 12.21 Mb, both arbitrary points on a plateau, and never the
## 13.81 Mb sub-peak that sits nearest the pooled GWAS peak. A 5% drop interval
## cannot represent a plateau, so the honest presentation is one interval per
## chromosome plus the caveat that on chromosome V the reported interval marks
## the top of a much wider region (LOD is still 490 at the pooled GWAS peak).
## The machinery is left in place should a different drop or gap be wanted.
##
## Bar opacity is the QTL's peak LOD on a square-root scale, so the strength of
## each interval is visible without a second axis. LOD spans 5 to 940 here, which
## is why the scale is not linear.
##
## Two figures are written. The main one keeps only QTL with peak LOD > 100,
## which is enough to carry every claim -- chromosome III in both tracks and both
## crosses, chromosome V and I in the mig-6 track only -- while dropping the
## clutter of barely-significant intervals. The supplement keeps every
## genome-wide significant QTL.
##
## A 1 Mb window either side of each significant pooled ("bulk") GWAS peak is
## shaded through the full height of the plot, tracks included, so the cross
## intervals can be read against the association signal directly.
##
## Intervals use a 5% LOD drop from each chromosome's peak marker, walking
## outward and stopping at the first marker below peak * 0.95. This is the same
## algorithm the export pipeline uses for its shipped intervals
## (N2-XZ_export/scripts/xqtl_pipeline.R, peak_intervals) at a tighter drop, so
## these are narrower than the 10% intervals in TABLE_cross_support_intervals.tsv.
## ---------------------------------------------------------------------------
...
LOD_MIN <- 100
kept <- build(LOD_MIN, "FIG17_gwas_with_cross_qtl_tracks",
              c(100, 300, 600, 900))
build(b$CROSS_THR, "FIG17S_gwas_with_all_cross_qtl",
      c(10, 100, 300, 600, 900))

cat("\n== intervals on the main figure (peak LOD > ", LOD_MIN, ") ==\n", sep = "")
