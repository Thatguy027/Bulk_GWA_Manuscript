...
##
## Spearman of -log10 p between a reconstruction and the published traits,
## computed on markers INSIDE each published interval only. Only correlations
## AGAINST THE TWO PUBLISHED TRAITS are shown; the full trait-by-trait matrix
...
REF <- c(published_slope_baugh = "vs published Slope",
         published_pc1_baugh   = "vs published PC1")
...
R <- rbindlist(lapply(seq_len(nrow(IV)), function(i) {
  ...
  rbindlist(lapply(names(REF), function(rf) CMP[, .(
    interval = IV$id[i], ref = REF[[rf]], cmp,
    r = sapply(trait, function(t)
...
print(dcast(R, interval + cmp ~ ref, value.var = "r")[, lapply(.SD, function(x)
  if (is.numeric(x)) round(x, 2) else x)], nrows = 30)
...
p <- ggplot(R, aes(r, cmp, colour = ref, shape = detects)) +
  ...
  geom_text(aes(label = sprintf("%.2f", r)), position = position_dodge(width = 0.55),
            vjust = -1.05, size = 2.2, show.legend = FALSE) +
  scale_colour_manual(values = c(`vs published Slope` = "#2E4057",
                                 `vs published PC1`   = "#C4302B")) +
  ...
  labs(title = "How closely each reconstruction tracks the published traits, inside each interval",
       subtitle = paste("Spearman of -log10 p across markers within the published interval.",
                        "Filled points clear Bonferroni somewhere in that interval, open points do not --",
                        "\ntracking the published signal and detecting it are different things."),
       x = "Spearman correlation to the published trait", y = NULL)
