...
a <- commandArgs(TRUE)
N_LOCI <- if (length(a) >= 1) as.integer(a[1]) else 12L   # causal loci per MAF bin
N_REP  <- if (length(a) >= 2) as.integer(a[2]) else 2L    # noise replicates per cell
## HERITABILITY GRID, sized by power rather than picked round. At n = 96 the 50%
## power point is h2 = 0.23 against a Bonferroni threshold of 6.94 and 0.16
## against the eigen threshold of 4.6, so the grid straddles both. The first
## version used 0.05, 0.15, 0.35: at 0.05 nothing detects (0.1% power at
## Bonferroni) and at 0.35 everything does (97%), so two of the three levels
## returned the same answer for every panel and could separate none of them.
H2     <- c(0.15, 0.22, 0.30)
## Nulls are for the inflation factor, not for an empirical threshold. Eight is
## far too few to put a 5% cutoff on the genome-wide maximum, but lambda is the
## median chi-squared over ~450k markers and is well determined from a handful
## of scans, so the cutoff comes from Bonferroni or eigen corrected by lambda.
N_NULL <- 8L
MAF_MIN <- 0.05
...
bim <- P$pruned_bim
bim[, id := sub("_.*$", "", id)]
## THREE PANELS, spanning the surrogate rather than sampling it: the top of the
## mappable-marker range, the bottom, and the one that matters most. "naive:
## most private" carries more private markers than any other panel and still
## sits at the random mean for mappable markers, so if it detects as well as the
## optimised panel the central claim of this branch is wrong.
##
## DROPPED, and why. The Baugh panel is intermediate and tests nothing the other
## three do not. The superseded K*-constrained panel differs from the current one
## by 2% of mappable markers, which is 0.5 sigma on 216 paired traits -- it
## cannot resolve, so it would only buy a null result that means nothing.
## POOL_SIM_PANELS overrides this if the full set is wanted.
KEEP <- strsplit(Sys.getenv("POOL_SIM_PANELS",
  "optimised|naive: most private|RNAi panel (93)"), "|", fixed = TRUE)[[1]]
memb <- fread("plots/diagnostics/pool_optimizer_panels.tsv")
PANELS <- split(memb$strain, memb$panel)
PANELS <- PANELS[intersect(KEEP, names(PANELS))]
stopifnot(length(PANELS) == length(KEEP))
