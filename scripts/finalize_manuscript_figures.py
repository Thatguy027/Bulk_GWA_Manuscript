...
PLOTS = ROOT / "plots"
CAPTIONS = ROOT / "FIGURE_CAPTIONS.txt"
...
MANIFEST = [
    ...
    ("Figure S9A","SUPP_FIG_XX_sid2_ortholog_conservation",        "SUPP_FIG_XX_sid2_ortholog_conservation",
     "the N94-C95-T96 sequon across the Elegans supergroup"),
    ("Figure S9B","SUPP_FIG_XX_sid2_allele_swaps_full",            "SUPP_FIG_XX_sid2_allele_swaps_full",
     "the full residue-96 and N94A allele-swap series"),
...
def main() -> int:
    ...
    print(f"resolved {len(resolved)} figures, all sources and captions present")
    if unused:
    ...
    md.append("Figures cited by the current draft, in citation order, each with the "
              "caption from `FIGURE_CAPTIONS.txt`.\n")
    md.append("PNGs are shown inline; a print-resolution PDF sits beside each one.\n")
    md.append("Generated by `scripts/finalize_manuscript_figures.py` -- edit the "
              "captions or the figure scripts and re-run rather than editing this "
              "file.\n")
    md.append("\n---\n")
    ...
    for label, base, png, pdf, cap, cited in sorted(resolved, key=lambda r: sort_key(r[0])):
        ...
        md.append(f"*Source: `plots/{base}` &middot; "
                  f"print version: [`{slug}.pdf`]({slug}.pdf)*\n")
        md.append(f"\n{cap}\n")
        md.append("\n---\n")
    ...
    if lint:
        ...
        print("Fix them in FIGURE_CAPTIONS.txt and re-run; this script will not edit prose.")
        for msg in lint:
            print(f"  {msg}")
    else:
        print("\ncaption lint: clean")
