...
ratios <- data.table::fread("data/qubit_dna_prep.csv", header = T) %>%
  tidyr::gather(col, ratio, -V1) %>%
  dplyr::rename(row = V1) %>%
  dplyr::mutate(well = paste0(row, col))

standards_wells <- c(paste0("E",7:12),
               paste0("F",1:12))
...
samples <- data.table::fread("data/qubit_dna_prep.csv", header = T) %>%
  dplyr::rename(row = V1) %>%
...
write.table(samples_conc, "data/qubit_dna_conc.csv", col.names = T, row.names = F, quote = F, sep = ",")
